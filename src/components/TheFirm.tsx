...
const TheFirm = () => {
  ...
  useGSAP(() => {
    ...
    gsap.fromTo(textLayer1Ref.current, 
      { x: -500, opacity: 0 },
      {
        x: 0,
    ...
    gsap.fromTo(textLayer2Ref.current, 
      { x: 500, opacity: 0 },
      {
        x: 0,
    ...
    tl.from(labelRef.current, { y: -50, opacity: 0, letterSpacing: "1em" }, 0);

    // Headline Top: Flies in from Left with Blur
    tl.from(headlineTopRef.current, { 
      x: -100, 
      y: -50, 
      opacity: 0, 
      filter: "blur(20px)",
    ...
    tl.from(headlineBottomRef.current, { 
      x: 100, 
      y: 50, 
      opacity: 0, 
      filter: "blur(20px)",
  ...
  return (
    <section ref={containerRef} className="relative min-h-[100dvh] bg-obsidian overflow-hidden flex items-center justify-center py-24 md:py-0">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
        <div className="mb-12">
          <span ref={labelRef} className="block text-silver-mid text-xs tracking-[0.4em] uppercase mb-4">The Philosophy</span>
          <h3 className="font-serif text-5xl md:text-7xl text-white leading-tight">
            <span ref={headlineTopRef} className="inline-block">We don't just sign.</span> <br />
            <span ref={headlineBottomRef} className="inline-block text-silver-metallic italic">We certify trust.</span>
          </h3>
        </div>

        <div className="grid md:grid-cols-2 gap-12 text-left">
          <p ref={paraLeftRef} className="font-sans text-gray-400 text-lg leading-relaxed">
            In a world of digital noise, the physical seal remains the ultimate anchor of truth. 
            Keystone Notary Group represents the gold standard in mobile facilitation.
          </p>
          <p ref={paraRightRef} className="font-sans text-gray-400 text-lg leading-relaxed">
            From multi-million dollar real estate closings to sensitive estate planning, 
            our presence ensures your transaction is unassailable.
          </p>
        </div>
      </div>
    </section>
