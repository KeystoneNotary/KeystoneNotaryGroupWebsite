...
const BookingSection = () => {
  ...
  return (
    <section
      id="booking"
      className="relative min-h-[80vh] bg-black text-platinum py-12 md:py-16 px-4 md:px-10 overflow-hidden"
    >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
                  {(availabilityState === "loading"
                    ? Array.from({ length: 10 }).map((_, idx) => (
                        <div
                          key={idx}
                          className="animate-pulse bg-neutral-900 h-12 rounded border border-neutral-800"
                        />
                      ))
                    : availableSlots
                  ).map((slot, idx) =>
